...
import { ArrowRight, Target, TrendingUp, ShieldCheck, Headphones, Cog, Bot, ClipboardCheck, Users, Wrench, Factory, Building2, Briefcase } from 'lucide-react';
      ...
      <div className="mx-auto flex max-w-[1536px] flex-wrap items-center justify-between gap-5 px-6 py-4 lg:px-8 xl:px-10">
        <button onClick={() => setPage('home')} className="flex items-center gap-4 text-left">
          <div>
            ...
            <div className="text-sm font-semibold uppercase tracking-[0.22em] text-blue-400 md:text-xl">Automated Solutions</div>
          </div>
        </button>
        ...
        <nav className="hidden flex-wrap items-center gap-8 text-[15px] font-bold uppercase tracking-wide text-white lg:flex xl:gap-12">
          {navItems.map(([key, label]) => (
            <button
              key={key}
              onClick={() => setPage(key)}
              className={`pb-2 transition ${page === key ? 'border-b-4 border-yellow-400 text-yellow-300' : 'hover:text-yellow-200'}`}
            >
              {label}
            </button>
          ))}
  ...
  return (
    <section className="overflow-hidden bg-[#020814] text-white">
      <div className="relative border-t border-white/10">
        ...
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(1,6,16,0.95)_0%,rgba(1,6,16,0.86)_30%,rgba(1,6,16,0.36)_58%,rgba(1,6,16,0.08)_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_30%,rgba(7,41,93,0.42),transparent_34%)]" />

        <div className="relative mx-auto grid min-h-[670px] max-w-[1536px] items-center px-6 py-10 lg:grid-cols-[0.78fr_1.22fr] lg:px-8 xl:px-10">
          <div className="max-w-[680px] pt-6 md:pt-10">
            <h1 className="text-[56px] font-black uppercase leading-[0.92] tracking-tight text-white sm:text-[72px] xl:text-[86px]">
              Automation
              <br />
              That Drives
              <br />
            ...
            <p className="mt-7 max-w-[620px] text-[22px] leading-[1.65] text-white/92 sm:text-[24px]">
              RCB Automation delivers custom automation solutions that improve efficiency, increase productivity, and drive measurable results.
            </p>
            <div className="mt-10 flex flex-wrap gap-5">
              <a
                ...
                className="inline-flex items-center gap-3 rounded-2xl bg-yellow-400 px-8 py-5 text-xl font-black uppercase tracking-wide text-slate-950 shadow-xl transition hover:brightness-105"
              >
              ...
              >
                Our Solutions <ArrowRight className="h-5 w-5" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="relative z-10 mx-auto -mt-1 grid max-w-[1536px] border-t border-white/10 bg-[#031326]/95 md:grid-cols-2 xl:grid-cols-4">
        {topFeatures.map((item, index) => {
          ...
          return (
            <div
              key={item.title}
              className={`flex gap-5 px-7 py-8 ${index < topFeatures.length - 1 ? 'xl:border-r xl:border-white/20' : ''} ${index < 2 ? 'md:border-r md:border-white/20 xl:border-r' : ''}`}
            >
              <div className="pt-1 text-yellow-400">
                <Icon className="h-12 w-12" strokeWidth={2.3} />
              </div>
              <div>
                <div className="text-[18px] font-black uppercase tracking-wide text-white">{item.title}</div>
  ...
  return (
    <section className="bg-[#081224] px-6 py-20 text-white lg:px-10 lg:py-24">
      <div className="mx-auto max-w-[1500px]">
        <div className="text-xl font-semibold uppercase tracking-[0.18em] text-yellow-400">RCB Automation</div>
        <h1 className="mt-5 text-[46px] font-black uppercase tracking-tight md:text-[64px]">{pages[page]}</h1>
    ...
    <section id="welcome" className="bg-[#efefef] px-6 py-20 lg:px-10 lg:py-24">
      <div className="mx-auto max-w-[1536px]">
        <div className="mx-auto max-w-[980px] text-center">
          <div className="text-[18px] font-bold uppercase tracking-[0.22em] text-yellow-500">Welcome to RCB Automation</div>
        ...
        <div className="mt-20 grid gap-y-12 border-t border-slate-300 pt-14 md:grid-cols-2 xl:grid-cols-4">
          {lowerFeatures.map((item, index) => {
            ...
            return (
              <div
                key={item.title}
                className={`px-6 ${index < lowerFeatures.length - 1 ? 'xl:border-r xl:border-slate-300' : ''} ${index < 2 ? 'md:border-r md:border-slate-300 xl:border-r' : ''}`}
              >
      ...
      <div className="mx-auto max-w-[1500px] rounded-[32px] bg-white p-10 shadow-sm lg:p-14">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <h2 className="mt-4 text-[52px] font-black uppercase leading-tight text-[#081224]">Built for real industrial projects</h2>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {services.map((item) => (
              <div key={item} className="rounded-2xl border border-slate-200 bg-slate-50 px-6 py-5 text-xl font-semibold text-[#111827]">
                {item}
              </div>
            ))}
          </div>
    ...
    <section className="bg-[#f3f4f6] px-6 py-20 lg:px-10 lg:py-24">
      <div className="mx-auto max-w-[1200px] rounded-[32px] bg-white p-10 shadow-sm lg:p-16">
        <div className="flex items-center gap-4 text-blue-600">
          ...
          <div className="text-xl font-semibold uppercase tracking-[0.18em] text-yellow-500">About RCB Automation</div>
        </div>
        <h2 className="mt-6 text-[58px] font-black uppercase leading-tight text-[#081224]">Engineering-first automation partnership</h2>
      ...
      <div className="mx-auto grid max-w-[1500px] gap-8 lg:grid-cols-3">
        {caseStudies.map((item) => (
          <div key={item.title} className="rounded-[30px] bg-white p-10 shadow-sm">
            <div className="text-xl font-semibold uppercase tracking-[0.18em] text-yellow-500">Project Highlight</div>
            <h3 className="mt-5 text-[34px] font-black uppercase leading-tight text-[#081224]">{item.title}</h3>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="rounded-[28px] p-4 lg:p-6">
            ...
            <div className="mt-12 space-y-5">
              {[
                ...
                <div key={item} className="flex items-center gap-5 rounded-[24px] border border-white/10 bg-white/6 px-5 py-5 shadow-lg shadow-black/10">
                  ...
                  <div className="text-[18px] font-medium leading-8 text-white md:text-[20px]">{item}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[28px] p-2">
            <form className="grid gap-5">
              <input placeholder="Name" className="h-[94px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 text-[20px] text-white outline-none placeholder:text-white/45" />
              <input placeholder="Company" className="h-[94px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 text-[20px] text-white outline-none placeholder:text-white/45" />
              <div className="grid gap-5 md:grid-cols-2">
                <input placeholder="Email" className="h-[94px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 text-[20px] text-white outline-none placeholder:text-white/45" />
                <input placeholder="Phone" className="h-[94px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 text-[20px] text-white outline-none placeholder:text-white/45" />
              </div>
              <input placeholder="Project Type" className="h-[94px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 text-[20px] text-white outline-none placeholder:text-white/45" />
              <textarea placeholder="Tell us about your automation goals" className="min-h-[220px] rounded-[26px] border border-white/10 bg-[#020b1f] px-6 py-6 text-[20px] text-white outline-none placeholder:text-white/45" />
              <button type="button" className="mt-1 h-[92px] rounded-[26px] bg-[#68a4f2] text-[22px] font-black text-[#041120] shadow-xl transition hover:brightness-105">
                Send Inquiry
              </button>
            </form>
          </div>
...
function ContactBand() {
  return (
    <section id="contact" className="bg-[#081224] px-6 py-16 text-white lg:px-10">
      <div className="mx-auto flex max-w-[1500px] flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
        <div>
          <div className="text-4xl font-black uppercase tracking-tight">Ready to Talk About Your Automation Project?</div>
          <p className="mt-3 text-xl text-white/80">Call +1 281 627 9833 or email rbath@rcbautomation.com</p>
        </div>
        <a href="mailto:rbath@rcbautomation.com" className="rounded-2xl bg-yellow-400 px-8 py-5 text-lg font-bold uppercase tracking-wide text-slate-950 transition hover:brightness-105">
          Contact Us
        </a>
      </div>
    </section>
