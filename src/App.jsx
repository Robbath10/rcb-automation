  ...
  return (
    <section style={lightSection}>
      <div style={{ ...contentWrap, display: 'grid', gap: 28, gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', alignItems: 'stretch' }}>
        {industries.map((item) => <div key={item} style={{ ...whiteCard, minHeight: 360, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}><div style={industryIconWrap}><Icon name="building" size={34} strokeWidth={2.4} /></div><h3 style={{ ...cardTitle, marginTop: 18, lineHeight: 1.15 }}>{item}</h3><p style={cardText}>Automation strategies tailored for performance, reliability, operator usability, and measurable production improvement.</p></div>)}
      </div>
    </section>
  );
}
...
function AboutPage() {
  return <section style={lightSection}><div style={{ ...splitPanel, maxWidth: 1200 }}><div style={{ display: 'flex', alignItems: 'center', gap: 16, color: '#2563eb' }}><Icon name="briefcase" size={42} /><div style={sectionEyebrowLight}>About RCB Automation</div></div><h2 style={{ ...sectionTitleLeft, marginTop: 20 }}>Engineering-first automation partnership</h2><p style={leftLead}>RCB Automation helps manufacturers improve efficiency, controls visibility, safety, and long-term performance through custom industrial automation solutions. Our work is centered on dependable delivery, clear communication, and results that support growth.</p><p style={{ ...leftLead, marginTop: 18 }}>We partner with clients from concept through commissioning and continue supporting them as systems evolve, expand, and scale.</p></div></section>;
}
