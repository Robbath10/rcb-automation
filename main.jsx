...
import { ArrowRight, Target, TrendingUp, ShieldCheck, Headphones, Cog, Bot, ClipboardCheck, Users, Wrench, Factory, Building2, Briefcase, BadgeCheck, Globe2, Cpu, ChevronRight } from 'lucide-react';
        ...
        <div className="relative mx-auto grid min-h-[760px] max-w-[1536px] items-center px-6 py-12 lg:grid-cols-[0.76fr_1.24fr] lg:px-8 xl:px-10">
          <div className="max-w-[700px] pt-8 md:pt-12">
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {eliteStats.map((item) => (
                <div key={item.label} className="rounded-[22px] border border-white/10 bg-white/10 px-5 py-5 backdrop-blur-sm">
                  <div className="text-[26px] font-black text-white">{item.value}</div>
                  <div className="mt-1 text-[12px] font-bold uppercase tracking-[0.22em] text-white/60">{item.label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-[1536px]">
        <div className="grid gap-6 lg:grid-cols-3">
          {eliteHighlights.map((item) => {
            const Icon = item.icon;
            return (
              <div key={item.title} className="rounded-[30px] border border-white/10 bg-white/5 p-8 shadow-xl shadow-black/10 backdrop-blur-sm">
...
function ContactPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const subject = encodeURIComponent(
      `RCB Automation Inquiry${form.projectType ? ` - ${form.projectType}` : ''}`
    );
...
${form.message}`
    ...
    const mailtoLink = `mailto:rbath@rcbautomation.com?subject=${subject}&body=${body}`;
      ...
      <div className="mx-auto max-w-[1536px] rounded-[34px] border border-white/10 bg-[linear-gradient(135deg,#081224_0%,#0c1a30_55%,#0a1730_100%)] px-8 py-10 shadow-2xl shadow-black/20 lg:px-12 lg:py-12">
        <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
          <div className="max-w-[900px]">
            <div className="text-[16px] font-bold uppercase tracking-[0.24em] text-yellow-400">Ready to Build</div>
            <div className="mt-3 text-4xl font-black uppercase tracking-tight text-white md:text-5xl">Let’s talk about your next automation project</div>
            <p className="mt-4 text-xl leading-9 text-white/75 md:text-2xl">Talk with RCB Automation about robotic integration, PLC upgrades, controls troubleshooting, retrofits, or full turn-key systems.</p>
          </div>
          <div className="flex flex-wrap gap-4">
            <button onClick={() => setPage('contact')} className="rounded-2xl bg-yellow-400 px-7 py-4 text-base font-black uppercase tracking-wide text-slate-950 shadow-lg transition hover:brightness-105 lg:px-8 lg:text-lg">
              Contact Us
            </button>
            <a href="tel:+12816279833" aria-label="Call RCB Automation now" className="rounded-2xl border border-white/15 bg-white/5 px-8 py-5 text-lg font-black uppercase tracking-wide text-white transition hover:bg-white/10">
              Call Now
            </a>
          </div>
        </div>
